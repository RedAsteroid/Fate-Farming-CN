...
- GetAetherytesInZone() 在多线程访问时会引起游戏崩溃，具体表现为使用多地图脚本伐木时，传送到不同的地图后重新运行 Fate Farming 脚本时游戏崩溃。**已知与 Daily Routines 插件本体冲突，如果您需要`多地图伐木`，请务必禁用此插件避免崩溃发生，如果仍有崩溃发生请自行排查异常插件。**
